...
pub fn seed_crash_reports(storage_id: StorageId) -> Vec<CrashReport> {
    ...
    vec![
        CrashReport{storage_id,id:"crash-sodium-optifine".into(),instance_id:"inst-fog".into(),when:"2 minutes ago".into(),exit_code:1,lines:Some({let mut x=header();x.extend(["java.lang.RuntimeException: Mixin apply failed net.minecraft.class_310","Mixin apply for mod optifine failed","  at net.fabricmc.loader.impl.launch.knot.KnotClassDelegate.getPostMixinClassByteArray(KnotClassDelegate.java:427)","  at net.optifine.reflect.Reflector.<clinit>(Reflector.java:155)","  at me.jellysquid.mods.sodium.client.SodiumClientMod.onInitializeClient(SodiumClientMod.java:52)","Caused by: org.spongepowered.asm.mixin.transformer.throwables.MixinTransformerError: An unexpected critical error was encountered","  at org.spongepowered.asm.mixin.transformer.MixinProcessor.applyMixins(MixinProcessor.java:392)",""].into_iter().map(String::from));x}),stderr_tail:vec![],hs_err_path:None,findings:vec![finding("MIXIN_APPLY_MOD_FAILED","OptiFine failed while its mixin was being applied","The mod id appears directly in the matched crash signature. Disable or update OptiFine before retrying.",vec![7],vec!["m-optifine"])],environment:fog},
        CrashReport{storage_id,id:"crash-possible".into(),instance_id:"inst-neo".into(),when:"18 minutes ago".into(),exit_code:1,lines:Some(vec!["---- Minecraft Crash Report ----".into(),"// Oops. The game did not enjoy that.".into(),"".into(),"Time: 2026-08-17 14:22:18".into(),"Description: Initializing game".into(),"".into(),"java.lang.NoSuchMethodError: 'void net.neoforged.fml.ModList.registerEventHandler()'".into(),"  at dev.ftb.mods.ftbchunks.FTBChunks.init(FTBChunks.java:84)".into(),"  at net.neoforged.fml.ModLoader.gatherAndInitializeMods(ModLoader.java:203)".into(),"Caused by: java.lang.IllegalStateException: Could not load mod ftbquests".into(),"  at dev.ftb.mods.ftbquests.FTBQuestsCommon.init(FTBQuestsCommon.java:66)".into()]),stderr_tail:vec![],hs_err_path:None,findings:vec![finding("NO_SUCH_METHOD_ERROR","A required method was not available","This signature can indicate a missing or incompatible mod or game component. It does not identify one mod by itself.",vec![7],vec![])],environment:neo},
        CrashReport{storage_id,id:"crash-unknown".into(),instance_id:"inst-vanilla".into(),when:"yesterday".into(),exit_code:1,lines:Some(vec!["---- Minecraft Crash Report ----".into(),"// Oops. The game did not enjoy that.".into(),"".into(),"Time: 2026-08-17 14:22:18".into(),"Description: Initializing game".into(),"".into(),"java.lang.IllegalStateException: Failed to load registries due to above errors".into(),"  at net.minecraft.server.Bootstrap.bootStrap(Bootstrap.java:124)".into(),"  at net.minecraft.client.main.Main.main(Main.java:201)".into(),"Caused by: java.io.IOException: Error reading level.dat".into(),"  at net.minecraft.world.level.storage.LevelStorageSource.readLevelData(LevelStorageSource.java:301)".into(),"  at net.minecraft.client.Minecraft.createLevel(Minecraft.java:517)".into()]),stderr_tail:vec![],hs_err_path:None,findings:vec![],environment:vanilla},
