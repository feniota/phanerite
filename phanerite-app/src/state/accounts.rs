    ...
    pub fn from_account(account: &Account) -> Self {
        ...
        let (username, account_type, auth_server, active_profile_id, profiles) = match account {
            ...
            Account::Microsoft(auth) => {
                let profile = gpui_kit::block_on(auth.profile());
            ...
            Account::Yggdrasil(auth) => {
                let profiles = gpui_kit::block_on(auth.available_profiles());
                let selected = gpui_kit::block_on(auth.selected_profile());
...
impl AccountStore {
    pub fn new(accounts: Vec<Account>) -> Self {
        ...
        for account in accounts {
            ...
            assert!(
                gpui_kit::block_on(store.accounts.insert(key, account)).is_ok(),
    ...
    pub fn add(
        ...
        if gpui_kit::block_on(self.accounts.insert(key.clone(), account)).is_err() {
            return None;
        }
        self.active_id = Some(key);
        self.revision += 1;
    ...
    pub fn remove(&mut self, id: &str) -> bool {
        ...
        if !gpui_kit::block_on(self.accounts.remove(&key)) {
            return false;
        }
        if self.active_id.as_ref() == Some(&key) {
            self.active_id = None;
